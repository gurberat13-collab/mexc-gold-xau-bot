...
async def main() -> None:
    ...
    scanner = ScannerEngine(
        CONFIG, client, strategy, risk, wallet,
        executor, position_manager, logger, dummy_notifier,
    )

    if not CONFIG.telegram_token:
        logger.warning("TELEGRAM_TOKEN yok. Sadece terminal/log ile calisacak.")
        await scanner.start()
        await scanner.run_forever()
        return

    # FIX: risk parametresi TelegramController'a iletiliyor
